...
def prepare_context(raw_args: list[str]) -> None:
    parser = argparse.ArgumentParser(
        prog=f"{NAME_CLI}-status",
        description="Provisioner Status Info",
...
def main() -> int:
    ...
    try:
        ...
        from provisioner.cli.status import main as main_prog  # noqa: PLC0415
        ...
        host = ProvisionHost()
        return main_prog(host=host, show_paths=True).code
